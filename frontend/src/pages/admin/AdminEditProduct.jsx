...
export default function AdminEditProduct() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const res = await axios.get(`https://stockflow-wms-backend.onrender.com/api/products/${id}`, {
  ...
  const handleSave = async (e) => {
    ...
    try {
      const token = localStorage.getItem("sf_token");
      await axios.put(`https://stockflow-wms-backend.onrender.com/api/products/${id}`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDelete = async () => {
    if (window.confirm("Delete this product permanently? This cannot be undone.")) {
      try {
        const token = localStorage.getItem("sf_token");
        await axios.delete(`https://stockflow-wms-backend.onrender.com/api/products/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
